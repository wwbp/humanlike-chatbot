...
class TestFollowupFunctionality(TestCase):
    ...
    def test_followup_functionality(self):
        ...
        print("0. Test bot created:")
        print(f"   Test bot: {BOT_NAME}")
        print(f"   Idle time: {self.bot.idle_time_minutes} minutes")
        print(f"   Follow-up enabled: {self.bot.follow_up_on_idle}")
        ...
        response = self.client.post('/api/initialize_conversation/', 
                                  data=json.dumps(init_data),
                                  content_type='application/json')
        
        self.assertEqual(response.status_code, 200, f"Failed to initialize conversation: {response.content}")
        ...
        if last_user_message:
            ...
            print("✅ User message timestamp set to 2 minutes ago")
        ...
        response = self.client.post('/api/followup/',
                                  data=json.dumps(followup_data),
                                  content_type='application/json')
        
        self.assertEqual(response.status_code, 200, f"Follow-up request failed: {response.content}")
        ...
        self.assertEqual(response.status_code, 200, f"Failed to get bot configuration: {response.content}")
