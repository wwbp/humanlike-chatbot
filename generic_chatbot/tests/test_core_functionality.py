...
from chatbot.services.runchat import run_chat_round, generate_followup_message, is_user_idle
...
class TestCoreChatFunctionality(TestCase):
    ...
    async def test_run_chat_round_moderation_blocked(self, mock_moderate, mock_engine):
        ...
        self.assertIn("blocked by moderation", response)
        self.assertIn("Inappropriate content", response)
    ...
    def test_run_chat_round_function_exists(self):
        ...
        self.assertTrue(callable(run_chat_round))
        self.assertTrue(asyncio.iscoroutinefunction(run_chat_round))
...
class TestFollowupFunctionality(TestCase):
    ...
    async def test_is_user_idle_true(self):
        ...
        self.assertTrue(is_idle, f"Expected idle=True, got {is_idle}. Last message time: {old_time}, current time: {timezone.now()}")
    ...
    async def test_generate_followup_message_success(self, mock_run_chat):
        ...
        self.assertIsNone(error, f"Expected no error, got: {error}")
...
class TestIntegrationScenarios(TestCase):
    ...
    def test_bot_factory_creates_valid_bot(self):
        ...
        self.assertEqual(self.bot.name, "IntegrationBot")
        self.assertTrue(self.bot.follow_up_on_idle)
        self.assertEqual(self.bot.idle_time_minutes, 2)
    ...
    def test_conversation_factory_creates_valid_conversation(self):
        ...
        self.assertEqual(self.conversation.conversation_id, "integration-conv-789")
        self.assertEqual(self.conversation.bot_name, "IntegrationBot")
    ...
    def test_persona_relationship(self):
        ...
        self.assertIn(persona, self.bot.personas.all())
        self.assertEqual(persona.instructions, "Test instructions")
    ...
    async def test_utterance_creation_and_retrieval(self):
        ...
        self.assertEqual(retrieved.text, "Test message")
        self.assertEqual(retrieved.speaker_id, "user")
...
class TestEdgeCases(TestCase):
    ...
    async def test_followup_exactly_at_idle_threshold(self):
        ...
        self.assertTrue(is_idle, f"Expected idle=True, got {is_idle}. Message time: {idle_threshold}, current time: {timezone.now()}")
...
class TestAPIEndpoints(TestCase):
    ...
    def test_followup_api_endpoint_exists(self):
        ...
        self.assertTrue(True)  # Placeholder - would test actual endpoint
